...
class Api_Wrapping extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('api_model_wrapping');
...
public function wrappings()
{
	...
	$wrappings = $this->api_model_wrapping->get_wrappings($featured=false, $recentpost=false);
...
public function featured_wrappings()
{
	...
	$wrappings = $this->api_model_wrapping->get_wrappings($featured=true, $recentpost=false);
...
public function wrapping($id)
{
	...
	$wrapping = $this->api_model_wrapping->get_wrapping($id);
...
public function recent_wrappings()
{
	...
	$wrappings = $this->api_model_wrapping->get_wrappings($featured=false, $recentpost=5);
...
public function adminWrappings()
{
	...
	$posts = array();
	if($isValidToken) {
		$wrappings = $this->api_model_wrapping->get_admin_wrappings();
		foreach($wrappings as $wrapping) {
			$posts[] = array(
				'id' => $wrapping->id,
				'title' => $wrapping->title,
...
public function adminWrapping($id)
{
	...
	if($isValidToken) {
		...
		$wrapping = $this->api_model_wrapping->get_admin_wrapping($id);
...
public function createWrapping()
{
	...
	if($isValidToken) {
		...
		if( ! $isUploadError) {
			...
			$id = $this->api_model_wrapping->insertWrapping($wrappingData);
...
public function updateWrapping($id)
{
	...
	if($isValidToken) {
		...
		if( ! $isUploadError) {
			...
			$this->api_model_wrapping->updateWrapping($id, $wrappingData);
...
public function deleteWrapping($id)
{
	...
	if($isValidToken) {
		...
		$this->api_model_wrapping->deleteWrapping($id);
